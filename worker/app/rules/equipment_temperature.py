...
class EquipmentTemperatureRule:
    ...
    def evaluate(self, session: Session, context: RuleContext | None = None) -> list[AlertCandidate]:
        rows = session.execute(
            ...
            .where(
                sensor_readings.c.metric_name == "temperature_c",
        ...
        for row in rows:
            ...
            candidates.append(
                AlertCandidate(
                    alert_code="EQUIPMENT_TEMPERATURE_HIGH",
                    station_id=int(station_id),
                    equipment_id=int(row.equipment_id),
                    severity="high",
                    title=f"High equipment temperature at {row.asset_tag}",
                    description=f"Temperature reading {row.value} C exceeded threshold {self.threshold} C.",
                    evidence_json={
